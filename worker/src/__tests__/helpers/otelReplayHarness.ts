...
/**
 * Runs the production OTEL event phase through a regular writer instance and
 * reads the rows persisted in an isolated Memory table.
 */
export async function runOtelReplay(
  ...
  try {
    ...
    writer = new ClickhouseWriter({
      client,
      tableNames: { [TableName.EventsFull]: tableName },
    });
